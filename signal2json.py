...
# create a dict of thread ids to titles for use with threads with no corresponding contact name
titles = {}
...
backups['contacts'] = query(
	...
	LEFT JOIN thread ON thread.recipient_ids  = recipient_preferences.recipient_ids
...
for item in backups['contacts']:
	try: # contact has a user assigned name
		...
		try: # contact does not have a user assigned name, but has a signal profile name
			...
			try: # contact has no name other than the thread name
